...
def rolling_maximum(
    ...
    """
    assert window > n_reject + 1, "window must be larger than n_reject+1 to have anything in the window!"
    n    = len(flux)
    half = window // 2
...
@partial(jax.jit, static_argnames=('window_narrow', 'window_wide', 
                                   'n_reject', 'n_knots', 'rigidity', 
...
def fit_continuum(
    ...
    left_edge_weight  = edge_boost * logistic_weights(1-left_score, p0=p0_edge, softness=softness_edge)
    right_edge_weight = edge_boost * logistic_weights(1-right_score, p0=p0_edge, softness=softness_edge)
    w_edge = (
        jnp.ones(n)
          .at[:edge_pixels].set(left_edge_weight)
          .at[-edge_pixels:].set(right_edge_weight)
    )
    ...
    D2   = (
        jnp.zeros((n_knots - 2, n_knots))
          .at[rows, rows    ].set( 1.0)
          .at[rows, rows + 1].set(-2.0)
          .at[rows, rows + 2].set( 1.0)
    )
    ...
    y = {'narrow': roll_narrow, 'envelope': envelope, 'flux': flux, 'wide': roll_wide}[fit_target]
